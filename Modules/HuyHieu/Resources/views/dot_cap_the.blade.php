                ...
                <div class="box box-primary">
                    ...
                    <div class="col-md-3 form-group mt-4">
                        <button type="button" class="btn btn-sm btn-info waves-effect waves-light mb-1"
                            ...
                            {{isset($first) ? 'CẬP NHẬT ĐỢT CẤP HUY HIỆU' : 'THÊM ĐỢT CẤP HUY HIỆU'}}</button>
                    </div>
                    ...
                    <div class="col-md-12">
                        <div class="row">
                            <div class="collapse {{isset($first) ? 'in' : '' }}" id="collapseExample">
                                <div class="row">
                                    <form role="form" action="@if($first){{route('capNhatCapHuyHieuDang',$first->id)}}@else{{route('themMoiDotCapHuyHieuDang')}}@endif"  method="post" enctype="multipart/form-data"
                                          id="myform">
                                        @csrf
                                        <div class="box-body">
                    ...
                    <div class="col-md-12" style="margin-top: 20px">
                        <div class="row">
                            <form action="{{route('DSDotCapHuyHieuDang')}}" method="get">
                        ...
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            @forelse($danh_sach as $key=>$data)
                                <tr>
                                    ...
                                    <td class="text-center" style="vertical-align: middle"><a href="{{route('cap-huy-hieu-dang.index','dot_cap='.$data->id)}}">{{date("d/m/Y", strtotime($data->dot_cap_the))}}</a></td>
                                    ...
                                    <td class="text-center">
                                        <form method="POST" action="{{route('xoaDotCap',$data->id)}}">
                                            ...
                                            <a class="btn-action btn btn-color-blue btn-icon btn-light btn-sm"
                                               href="{{route('DSDotCapHuyHieuDang','id='.$data->id)}}" role="button" title="Sửa">
